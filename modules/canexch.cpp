	...
	while (vegetation.isobj) {
		...
		if (ifsrlvary) {
			...
			for (int ly = 0; ly < NSOILLAYER; ly++) {
				max_indiv_avail_NH4 += indiv.rpc * 61.56 * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NH4;
				max_indiv_avail_NO3 += indiv.rpc * 61.56 * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NO3;
				max_indiv_avail_NH4_myco += indiv.rpc_myco * 61.56 * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NH4;
				max_indiv_avail_NO3_myco += indiv.rpc_myco * 61.56 * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NO3;
...
void pdemand(Patch& patch, Vegetation& vegetation) {
	...
	while (vegetation.isobj) {
		...
		if (ifsrlvary) {
			...
			for (int ly = 0; ly < NSOILLAYER; ly++) {
				max_indiv_avail += indiv.rpc * 61.56 * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * pmin_avail;
				max_indiv_avail_myco += indiv.rpc_myco * 61.56 * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * pmin_avail;
