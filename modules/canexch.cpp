...
void photosynthesis_nostress(Patch& patch, Climate& climate) {
	...
	while (vegetation.isobj) {
		...
		photosynthesis(ps_env, ps_stress, pft,
			pft.lambda_max, 1.0, 1.0, indiv.cmass_leaf_today() / indiv.cton_leaf_min, indiv.cmass_leaf_today() / indiv.ctop_leaf_min, -1,
			indiv.photosynthesis);
		...
		if (date.diurnal()) {
			...
			for (int i=0; i<date.subdaily; i++) {
				...
				//               ps_result);
				photosynthesis(ps_env, ps_stress, pft,
					pft.lambda_max, 1.0, 1.0, indiv.cmass_leaf_today() / indiv.cton_leaf_min, indiv.cmass_leaf_today() / indiv.ctop_leaf_min, indiv.photosynthesis.vm,
					ps_result);
...
void ndemand(Patch& patch, Vegetation& vegetation) {
	...
	while (vegetation.isobj) {
		...
		double norg_scale_NO3_myco = kNmin + norg_avail_myco / (norg_avail_myco + (gridcell.pft[indiv.pft.id].Km_no3 / 1.66));
		double norg_scale_NH4_myco = kNmin + norg_avail_myco / (norg_avail_myco + (gridcell.pft[indiv.pft.id].Km_nh4 / 1.66));
		...
		if (ifsrlvary) {
			...
			for (int ly = 0; ly < NSOILLAYER; ly++) {
				max_indiv_avail_NH4 += indiv.rpc / ((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NH4;
				max_indiv_avail_NO3 += indiv.rpc / ((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NO3;
				max_indiv_avail_NH4_myco += indiv.rpc_myco / ((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NH4;
				max_indiv_avail_NO3_myco += indiv.rpc_myco / ((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * nmin_avail_NO3;
				...
				max_indiv_avail_NH4_myco += indiv.rpc_myco * indiv.pft.rootdist[ly] * soil.Frac_water[ly] * nmin_avail_NH4;
				max_indiv_avail_NO3_myco += indiv.rpc_myco * indiv.pft.rootdist[ly] * soil.Frac_water[ly] * nmin_avail_NO3;*/
			/*	max_indiv_avail_NH4 += (indiv.rpc * 61.56 * indiv.pft.rootdist[ly]) / 
					((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * nmin_avail_NH4 * soil.Frac_water[ly];
				max_indiv_avail_NO3 += (indiv.rpc * 61.56 * indiv.pft.rootdist[ly]) /
					((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * nmin_avail_NO3 * soil.Frac_water[ly];
				max_indiv_avail_NH4_myco += (indiv.rpc_myco * 61.56 * indiv.pft.rootdist[ly]) /
					((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * nmin_avail_NH4 * soil.Frac_water[ly];
				max_indiv_avail_NO3_myco += (indiv.rpc_myco * 61.56 * indiv.pft.rootdist[ly]) /
					((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * nmin_avail_NO3 * soil.Frac_water[ly];*/
			}
			max_indiv_avail_org_myco = min(1.0, indiv.rpc_myco) * norg_avail_myco;
...
void pdemand(Patch& patch, Vegetation& vegetation) {
	...
	while (vegetation.isobj) {
		...
		if (ifsrlvary) {
			...
			for (int ly = 0; ly < NSOILLAYER; ly++) {
				max_indiv_avail += indiv.rpc / ((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * pmin_avail;
				max_indiv_avail_myco += indiv.rpc_myco / ((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * indiv.pft.rootdist[ly] * (((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) / patch.water_content()) * pmin_avail;
				/*max_indiv_avail += indiv.rpc * indiv.pft.rootdist[ly] * soil.Frac_water[ly] * pmin_avail;
				max_indiv_avail_myco += indiv.rpc_myco * indiv.pft.rootdist[ly] * soil.Frac_water[ly] * pmin_avail;*/
				//max_indiv_avail += (indiv.rpc * 61.56 * indiv.pft.rootdist[ly]) /
				//	((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * pmin_avail * soil.Frac_water[ly];
				//max_indiv_avail_myco += (indiv.rpc_myco * 61.56 * indiv.pft.rootdist[ly]) /
				//	((soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * soil.Dz[ly]) * pmin_avail * soil.Frac_water[ly];
			}
			max_indiv_avail_org_myco = min(1.0, indiv.rpc_myco) * porg_avail_myco;
		}
		...
		if (ifsrlvary) {
			// Uptake is sum of root P uptake and mycorrhiza P from AMF or EMF sources.
...
void npp(Patch& patch, Climate& climate, Vegetation& vegetation, const Day& day) {
	...
	while (vegetation.isobj) {
		...
		if (indiv.myco_type)
			indiv.dnpp -= (indiv.fractomax_nmyco + indiv.fractomax_pmyco) * 6e-4;
