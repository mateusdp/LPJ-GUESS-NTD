	...
	while (vegetation.isobj) {
		...
		if (ifsrlvary) {
			...
			for (int ly = 0; ly < NSOILLAYER; ly++) {
				...
				max_indiv_avail_NH4_myco += indiv.rpc * indiv.pft.rootdist[ly] * (soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * nmin_avail_NH4;
				max_indiv_avail_NO3_myco += indiv.rpc * indiv.pft.rootdist[ly] * (soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * nmin_avail_NO3;
			}
			max_indiv_avail_org_myco = min(1.0, indiv.rpc_myco) * norg_avail_myco;
	...
	while (vegetation.isobj) {
		...
		if (ifsrlvary) {
			...
			for (int ly = 0; ly < NSOILLAYER; ly++) {
				...
				max_indiv_avail_myco += indiv.rpc * indiv.pft.rootdist[ly] * (soil.Frac_ice[ly] + soil.Frac_water[ly] + soil.Fpwp_ref[ly]) * pmin_avail;
			}
			max_indiv_avail_org_myco = min(1.0, indiv.rpc_myco) * porg_avail_myco;
		}
		else {	
