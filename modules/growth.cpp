	...
	if (indiv.pft.lifeform == TREE) {
		...
		if (!negligible(indiv.crownarea)) {
			...
			if (ifsrlvary) {
				...
				indiv.rpc = indiv.srl * indiv.d_root * PI  * indiv.cmass_root * indiv.densindiv * (1.0 - lambertbeer(indiv.srl * indiv.d_root * PI  * indiv.cmass_root * indiv.densindiv));
				indiv.rpc_myco = indiv.cmass_myco * 2.0e-6 * PI * 2.55e9 * indiv.densindiv * (1.0 - lambertbeer(indiv.cmass_myco * 2.0e-6 * PI * 2.55e9 * indiv.densindiv));
		...
		if(indiv.pft.landcover != CROPLAND) {
			...
			if (!negligible(indiv.cmass_leaf)) {
				...
				if (ifsrlvary) {
					...
					indiv.rpc = 1.0 - lambertbeer(indiv.srl * indiv.d_root * PI  * indiv.cmass_root);
