	...
	if (indiv.pft.lifeform == TREE) {
		...
		if (!negligible(indiv.crownarea)) {
			...
			if (ifsrlvary) {
				...
				indiv.rpc = indiv.srl * indiv.d_root * indiv.cmass_root * PI * 123.120 / indiv.vegetation.patch.soil.soiltype.wtot;
				indiv.rpc_myco = indiv.cmass_myco * 2.6e9 * 2e-6 * PI * 123.120 / indiv.vegetation.patch.soil.soiltype.wtot;
