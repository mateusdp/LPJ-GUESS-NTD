...
CommonOutput::CommonOutput() {
	...
	declare_parameter("file_cmass_amf", &file_cmass_amf, 300, "C arbuscular mycorrhizal fungi biomass output file");
	declare_parameter("file_cmass_ecm", &file_cmass_ecm, 300, "C ecto mycorrhizal fungi biomass output file");
...
void CommonOutput::define_output_tables() {
	...
	ColumnDescriptors srl_columns;
	srl_columns += ColumnDescriptors(pfts, 13, 1);
	...
	create_output_table(out_cmass_amf,			file_cmass_amf,			 cmass_myco_columns);
	create_output_table(out_cmass_ecm, file_cmass_ecm, cmass_myco_columns);
...
void CommonOutput::outannual(Gridcell& gridcell) {
	...
	double landcover_cmass_amf[NLANDCOVERTYPES] = { 0.0 };
	double landcover_cmass_ecm[NLANDCOVERTYPES] = { 0.0 };
	...
	double mean_standpft_cmass_amf = 0.0;
	double mean_standpft_cmass_ecm = 0.0;
	...
	double mean_standpft_srl = 0.0;

	double cmass_gridcell=0.0;
	double cmass_amf_gridcell = 0.0;
	double cmass_ecm_gridcell = 0.0;
	...
	double standpft_cmass_amf = 0.0;
	double standpft_cmass_ecm = 0.0;
	...
	while (pftlist.isobj) {
		...
		mean_standpft_cmass_amf = 0.0;
		mean_standpft_cmass_ecm = 0.0;
		...
		while (gc_itr != gridcell.end()) {
			...
			// Sum C biomass, NPP, LAI and BVOC fluxes across patches and PFTs
			standpft_cmass=0.0;
			standpft_cmass_amf = 0.0;
			standpft_cmass_ecm = 0.0;
			standpft_nmass=0.0;
			standpft_pmass = 0.0;
			...
			while (stand.isobj) {
						...
						if (indiv.id!=-1 && indiv.alive) {
							...
							if (indiv.pft.id==pft.id) {
								...
								if (pft.lifeform==TREE) {
									...
									standpft_srl += indiv.srl / patcharea;
								}
								if (ifsrlvary) {
									if(indiv.myco_type)
										standpft_cmass_ecm += indiv.cmass_myco;
									else
										standpft_cmass_amf += indiv.cmass_myco;
								}
				...
				standpft_cmass_amf /= (double)stand.npatch();
				standpft_cmass_ecm /= (double)stand.npatch();
				...
				standpft_srl /= (double)stand.npatch();
				...
				landcover_cmass_amf[stand.landcover] += standpft_cmass_amf * stand.get_landcover_fraction();
				landcover_cmass_ecm[stand.landcover] += standpft_cmass_ecm * stand.get_landcover_fraction();
				...
				if(active_fraction) {
					...
					mean_standpft_cmass_amf += standpft_cmass_amf * stand.get_gridcell_fraction() / active_fraction;
					mean_standpft_cmass_ecm += standpft_cmass_ecm * stand.get_gridcell_fraction() / active_fraction;
					...
					mean_standpft_srl += standpft_srl * stand.get_gridcell_fraction() / active_fraction;
				}

				//Update stand totals
				stand.anpp += standpft_anpp;
				stand.cmass += standpft_cmass;
				stand.cmass_myco += standpft_cmass_amf + standpft_cmass_ecm;
				...
				cmass_amf_gridcell += standpft_cmass_amf * fraction_of_gridcell;
				cmass_ecm_gridcell += standpft_cmass_ecm * fraction_of_gridcell;
				...
				srl_gridcell += standpft_srl * fraction_of_gridcell;
				...
				if (!(date.year%PLOT_INTERVAL)) {
					...
					if (mean_standpft_cmass_leaf > 0.0 && ifnlim) {
						...
						if (ifsrlvary) {
							...
							plot("C mass AMF [kgC/ha]", pft.name, date.year, mean_standpft_cmass_amf * M2_PER_HA);
							plot("C mass EcM [kgC/ha]", pft.name, date.year, mean_standpft_cmass_ecm * M2_PER_HA);
						}
						if (ifslavary) {
							if (mean_standpft_densindiv_total > 0.0)
		...
		if (mean_standpft_densindiv_total > 0.0) {
			...
			outlimit(out, out_srl, mean_standpft_srl / mean_standpft_densindiv_total);
		}
		else {
			outlimit(out, out_sla, 0.0);
			outlimit(out, out_wsg, 0.0);
			outlimit(out, out_srl, 0.0);
		}
	...
	outlimit(out, out_cmass_amf, cmass_amf_gridcell  * M2_PER_HA);
	outlimit(out, out_cmass_ecm, cmass_ecm_gridcell  * M2_PER_HA);
	...
	if (dens_gridcell > 0.0) {
		...
		outlimit(out, out_srl, srl_gridcell / dens_gridcell);
	}
	else {
		outlimit(out, out_sla, 0.0);
		outlimit(out, out_wsg, 0.0);
	...
	if (run_landcover) {
		for(int i=0;i<NLANDCOVERTYPES;i++) {
			if(run[i]) {
				...
				outlimit(out, out_cmass_amf, landcover_cmass_amf[i]);
				outlimit(out, out_cmass_ecm, landcover_cmass_ecm[i]);
